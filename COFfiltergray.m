function [ text,rres ] = COFfiltergray( img,hws )
...
% hws     = hwss;    % filter - half window size default 3
...
% 这个f就是统计共生矩阵用到的高斯滤波 pab=C(a,b);分母为h(a)h(b)
...
f = fspecial('gaussian',  hws, hws/4); 
% ff = fspecial('average', [hws,hws]);

%%%%%
temp = coFilter(im.lab, im.idx, pmi, f);
res = lab2rgb(temp);
%%%%%
% res = lab2rgb(coFilter(im.lab, im.idx, pmi, f)) ;%f是滤波用到的高斯滤波
